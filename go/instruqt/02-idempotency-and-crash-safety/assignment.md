...
teaser: A retried delivery lands three times; watch the duplicates, then dedupe them
  with a one-line idempotency key.
notes:
- type: text
  contents: |
    # Making retries safe with idempotency (Go)
...
Temporal guarantees an Activity runs to completion *at least once*. If the Activity returns an error before it succeeds, Temporal retries the entire Activity body. Any side effect inside the body (a network call, a database write, a charge) runs again on every attempt.

You need an idempotency key when the Activity calls an external system and that call is not safe to repeat. The pattern:

1. Derive a key from stable input, not from a random value generated inside the Activity. `req.EventID` is stable across retries. A freshly generated UUID is not.
...
<iframe src="https://raw.githack.com/temporalio/edu-standalone-activities/main/docs/idempotency-demo/index.html" width="100%" height="520" frameborder="0" style="border: 0; border-radius: 8px; margin: 0.5em 0;"></iframe>
...
go run ./sendstandalone evt_buggy
```
...
Back in the [button label="Exercise" background="#444CE7"](tab-0) tab, find the `TODO` in `DeliverWebhook`. Add this line:
...
scripts/reset-receiver.sh
go run ./sendstandalone evt_fixed
```
...
Temporal's **default retry policy is unbounded**. It starts at 1 second, backs off up to 100 seconds, and has no maximum attempt count. Useful for transient failures, but risky for permanently broken receivers. Pass an explicit `RetryPolicy` to bound it:
...
For permanent failures the Activity itself can recognize, return `temporal.NewNonRetryableApplicationError(...)`. Temporal stops retrying immediately instead of using up your `MaximumAttempts`.
