...
- type: text
  contents: |
    ...
    two ways: directly via `client.ExecuteActivity` (the Standalone Activity path
    you've used all tutorial) and via `client.ExecuteWorkflow` (a Workflow that
...
1. Read `webhook/activity.go` and `webhook/workflow.go`. The Activity is the exact `DeliverWebhook` function from Module 01.
...
Open `webhook/activity.go` in the [button label="Exercise" background="#444CE7"](tab-0) tab. You'll recognize it. It's the `DeliverWebhook` Activity you wrote in Module 01:
...
Now open `webhook/workflow.go`. A short Workflow that calls the same Activity:
...
The Workflow calls `workflow.ExecuteActivity` with the same `DeliverWebhook` function the standalone caller invokes directly. The Activity does not know whether it is being invoked as a Standalone Activity or as a Workflow step.
...
go run ./sendviaworkflow evt_002
```
...
> **What's happening:** the Worker registered both the Activity and the Workflow. Temporal's server routes a `client.ExecuteActivity` call to dispatch the Activity directly; a `client.ExecuteWorkflow` call dispatches the Workflow, which then dispatches the Activity. Same code, two submission paths, two job types.
...
- The `ActivityIDConflictPolicy`, `WorkerActivitiesPerSecond`, and `Priority` knobs you learned still apply when those Activities are called from inside a Workflow.
...
What you can now do with Standalone Activities in Go:

- **`client.ExecuteActivity` / `client.StartActivity`**: submit an Activity as a top-level durable job, no Workflow required.
- **Idempotency keys for external writes**: make retries safe by giving the receiver a stable dedup key.
- **`ActivityIDConflictPolicy: enums.ACTIVITY_ID_CONFLICT_POLICY_USE_EXISTING`**: let the server handle duplicate `ExecuteActivity` calls instead of erroring.
- **`WorkerActivitiesPerSecond`** on the Worker: cap dispatch rate to protect the downstream service.
- **`Priority`** on `client.StartActivityOptions`: push urgent work ahead of bulk when the queue is contended.
- **`activity.RecordHeartbeat(ctx, progress)` + `HeartbeatTimeout`**: report progress from long-running jobs and resume from the last checkpoint after a crash.
