#!/usr/bin/env bash
# Content guardrails for the SAA tutorial (Go track).
#
...
# files (AGENTS.md). Go code is excluded - its doc comments are reviewed
# module-by-module rather than grep-gated.
...
HITS=$(grep -rnE \
  --include='*.md' --include='*.go' --include='*.svg' --include='*.html' --include='*.yml' \
...
# Both the standalone caller (sendstandalone/main.go) and the Workflow caller
# (webhook/workflow.go's WebhookWorkflow) must reference the SAME
# webhook.DeliverWebhook Activity. Checked in both exercise and solution.
INVARIANT_FAIL=0
for variant in exercise solution; do
  MOD06="$COURSE_DIR/$variant/06-same-code-runs-anywhere"
  STANDALONE="$MOD06/sendstandalone/main.go"
  WORKFLOW="$MOD06/webhook/workflow.go"

  if [ ! -f "$STANDALONE" ]; then
    echo "FAIL $STANDALONE missing"
    INVARIANT_FAIL=1
  elif ! grep -qE 'webhook\.DeliverWebhook\b' "$STANDALONE"; then
    echo "FAIL $STANDALONE does not call webhook.DeliverWebhook"
    INVARIANT_FAIL=1
  fi
  ...
  if [ ! -f "$WORKFLOW" ]; then
    ...
    if ! grep -qE 'func WebhookWorkflow\(' "$WORKFLOW"; then
      echo "FAIL $WORKFLOW does not define WebhookWorkflow"
      INVARIANT_FAIL=1
    fi
    if ! grep -qE 'workflow\.ExecuteActivity\(ctx, *DeliverWebhook\b' "$WORKFLOW"; then
      echo "FAIL $WORKFLOW's WebhookWorkflow does not call workflow.ExecuteActivity(ctx, DeliverWebhook, ...)"
      INVARIANT_FAIL=1
    fi
  fi
done
