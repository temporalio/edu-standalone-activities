...
- **Loading messages** (rotate during the ~20-60s sandbox provisioning): `lab_config.loadingMessages` is a list of plain strings, ~80 chars each. No markdown rendering. Aim for 10 entries. Mix concrete teaching with a touch of warmth.
...
- **A *Completed* Standalone Activity does not show that it was retried.** The Standalone Activities tab surfaces a climbing attempt counter and last failure only while the Activity is **Running**; once it finishes, the record reveals no retry history (and never shows app logs or a per-attempt breakdown). Do NOT write "now Completed, and its Attempt count shows it was retried" - that has regressed twice and is grep-gated by `verify-content.sh` check 5. To evidence a retry on a finished Activity, point learners at the **Worker console** logs (e.g. `temporalio.exceptions.ApplicationError: Simulated transient failure on attempt 1`) and the receiver's request count, not the UI record.
