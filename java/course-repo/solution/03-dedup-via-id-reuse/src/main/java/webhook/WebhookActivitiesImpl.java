...
public class WebhookActivitiesImpl implements WebhookActivities {
    ...
    public int deliverWebhook(WebhookDelivery request) {
        ...
        try {
            ...
            // Brief pause so SendDouble's second start call lands while this Activity is
            // still in flight. The id conflict policy only applies to in-flight duplicates;
            // once this returns, the id reuse policy governs instead.
            Thread.sleep(1000);
            return statusCode;
        } catch (IOException e) {
