...
/**
 * Call start() TWICE with the same activityId, back-to-back, simulating an upstream
 * system that sends the same logical event twice.
 * Without an ID conflict policy the second call raises an error. Fix: set idConflictPolicy to USE_EXISTING.
 */
public class SendDouble {
    ...
    public static void main(String[] args) {
        ...
        if (h1 != null) { h1.getResult(); System.out.println("[call-1] activity completed"); }
        if (h2 != null) { h2.getResult(); System.out.println("[call-2] activity completed"); }
    }
}
