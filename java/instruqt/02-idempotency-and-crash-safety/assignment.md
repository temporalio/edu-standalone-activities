...
- type: text
  contents: |
    # Making retries safe with idempotency (Java)
...
Temporal guarantees an Activity runs to completion *at least once*. If the Activity throws before it succeeds, Temporal retries the entire Activity body. Any side effect inside the body (a network call, a database write, a charge) runs again on every attempt.
...
3. Understand why the key has to be stable across retries (and not a freshly generated random value).

The **Solution** tab has the finished code. Estimated time: 10 minutes.

<iframe src="https://raw.githack.com/temporalio/edu-standalone-activities/java/docs/idempotency-demo/index.html" width="100%" height="520" frameborder="0" style="border: 0; border-radius: 8px; margin: 0.5em 0;"></iframe>

---

## 1. Reproduce the bug (~3 min)

Open `WebhookActivitiesImpl.java` in the [button label="Exercise" background="#444CE7"](tab-1) tab. The Activity is set up to POST the webhook, then throw a simulated transient error on its first two attempts. This simulates a transient failure after the side effect already happened: the receiver processed the request, but the Activity threw before Temporal heard "done." On each retry Temporal re-runs the Activity body, POST included.

There's a `TODO` just below the `Content-Type` header line. Leave it alone for now so you can see what goes wrong before the fix.
...
Temporal's **default retry policy is unbounded**. It starts at 1 second, backs off up to 100 seconds, and has no maximum attempt count. Useful for transient failures, but risky for permanently broken receivers. Pass an explicit `RetryOptions` to bound it (already set in this module's `SendStandalone.java`):
