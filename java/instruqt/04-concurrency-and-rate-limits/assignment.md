...
- type: text
  contents: |
    ...
    The fix is one option on the Worker: `setMaxTaskQueueActivitiesPerSecond`.
    The Worker dispatches Activities at the configured pace. Everything else
    waits in the Task Queue on the server.
    ...
    3. Add `setMaxTaskQueueActivitiesPerSecond(2)` to the Worker. Re-run with the rate-limited receiver. The flood of 429s stops.
...
Standalone Activities give you both controls in one place: `setMaxTaskQueueActivitiesPerSecond` paces dispatch so a fan-out does not overwhelm the receiver, and Priority puts urgent jobs ahead of bulk ones when the queue is contended.
...
time mvn -q compile exec:java -Dexec.mainClass=webhooks.SendBulk -Dexec.args="60"
```
...
Open the [button label="Temporal UI" background="#444CE7"](tab-5) tab, then **Standalone Activities**. All 60 `bulk-*` Activities should be **Completed** with start and end timestamps clustered in the same one- or two-second window.
...
Now send 60 deliveries against the rate-limited receiver using `SendBulkDemo` (`src/main/java/webhooks/SendBulkDemo.java`, which uses separate `demo-*` IDs so leftover retries don't collide with the `bulk-*` IDs used in sections 1 and 3):

```bash,run
# Reset and fan out 60 deliveries against the rate-limited receiver. Watch the 429s.
scripts/reset-receiver.sh
mvn -q compile exec:java -Dexec.mainClass=webhooks.SendBulkDemo -Dexec.args="60"
```

`SendBulkDemo` will hang because the Activities keep retrying on every 429. After about **5 seconds**, press **Ctrl+C**.

Check the [button label="Webhook receiver" background="#444CE7"](tab-4) tab. Only a handful of deliveries land at first; the rest get rejected with `429 Too Many Requests` and keep retrying. Open the [button label="Worker" background="#444CE7"](tab-3) tab and look for `HTTP 429` error lines.

Open the [button label="Temporal UI" background="#444CE7"](tab-5) tab, then **Standalone Activities**. Most of the `demo-*` Activities should be in **Running** state with the attempt count climbing.
...
Open `src/main/java/webhooks/Worker.java` in the [button label="Exercise" background="#444CE7"](tab-0) tab. There's a `TODO` comment inside the `WorkerOptions.newBuilder()` call. Add this option to the builder chain:
...
```bash,run
# Restart the Worker with setMaxTaskQueueActivitiesPerSecond(2)
mvn -q compile exec:java -Dexec.mainClass=webhooks.Worker
```
...
At 2/sec, draining 60 deliveries takes about **30 seconds**. Open the [button label="Temporal UI" background="#444CE7"](tab-5) tab, then **Standalone Activities**, and watch `bulk-*` Activities flip from **Running** to **Completed** about two per second.
...
`setMaxTaskQueueActivitiesPerSecond` controls *how fast* dispatch happens. The companion control is **Priority**, which decides *what order* work runs in when the queue is contended: a lower priority key means higher priority, so urgent jobs can jump ahead of a backlog even when they arrive later.
...
Two options, both `WorkerOptions` fields:

- `setMaxWorkerActivitiesPerSecond` is **per Worker**. The aggregate across your Worker fleet is `N × the per-Worker rate`.
- `setMaxTaskQueueActivitiesPerSecond` is **queue-wide**. It sets a hard cap regardless of Worker count. Use this when you can't predict how many Workers will be running.
