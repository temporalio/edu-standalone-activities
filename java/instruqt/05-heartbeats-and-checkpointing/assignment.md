...
- type: text
  contents: |
    ...
    next attempt reads ctx.getHeartbeatDetails(...) and resumes from the last
...
Standalone Activities include heartbeats and checkpointing. `ctx.heartbeat(progress)` reports liveness and stores progress on the Temporal server. When the next attempt starts, it reads `ctx.getHeartbeatDetails(Integer.class)` and resumes from there. No side database required.
...
Open `WebhookActivitiesImpl.java` in the [button label="Exercise" background="#444CE7"](tab-1) tab. The Activity already calls `ctx.heartbeat(delivered)` after each item, so the server _has_ the progress data. What's missing is the read on retry.
...
- [button label="Temporal UI" background="#444CE7"](tab-0), **Standalone Activities** view, `deliver-batch-10`: the Activity is still listed as **Running**. Temporal has not given up on it. It is waiting for a Worker to come back.
...
> **What's happening:** the Activity heartbeated its progress on the first attempt, but the second attempt never reads the heartbeat details. So it starts at `startIndex = 0` and redoes everything.
...
Without heartbeats for checkpoints, Temporal retries from zero, so you reprocess the 500 emails you already sent.
...
[x] You need the Activity to detect that it was cancelled mid-loop (heartbeat throws when cancellation is requested)

---

## 2. Read the checkpoint on retry (~2 min)

Back in the [button label="Exercise" background="#444CE7"](tab-1) tab, find the `TODO` in `deliverWebhookBatch`. Replace:
...
`getHeartbeatDetails(Integer.class)` returns whatever you passed to `ctx.heartbeat()` in the previous attempt, wrapped in an `Optional`. We pass a single number (`delivered`), so the checkpoint is the count of items already done.
...
- [button label="Temporal UI" background="#444CE7"](tab-0), **Standalone Activities** view, `deliver-batch-10`: still **Running**, waiting for a Worker.
...
The [button label="Webhook receiver" background="#444CE7"](tab-5) tab shows `"processed_count": 10`. No duplicates. The retry read the heartbeat details, jumped to the checkpoint index, and finished the remaining items without redoing anything.
...
Heartbeating also delivers **cancellation**. When someone runs `temporal activity cancel deliver-batch-10` (or an enclosing Workflow cancels), Temporal can't interrupt your Java code directly. It sets a flag on the server, and the next `ctx.heartbeat()` call throws an `ActivityCompletionException`.
...
try {
    for (int i = startIndex; i < req.getItems().size(); i++) {
        // ... do work ...
        ctx.heartbeat(delivered); // throws ActivityCompletionException if cancellation was requested
    }
} catch (ActivityCompletionException e) {
    log.info("Cancelled after {} deliveries", delivered);
    throw e; // rethrow so Temporal records the cancellation
}
```
...
> Your batch Activity has `heartbeatTimeout` of 5 seconds and processes one item per second. Mid-batch, the Worker hangs (deadlock, not crash). It stops calling heartbeat, but the process is still alive. What does Temporal do?
