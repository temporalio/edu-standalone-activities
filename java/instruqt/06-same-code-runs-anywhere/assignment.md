...
The Workflow proxies `deliverWebhook` from the same `WebhookActivities` interface the standalone caller submits. The Activity does not know whether it is being invoked as a Standalone Activity or as a Workflow step.
...
Open the [button label="Temporal UI" background="#444CE7"](tab-0) tab, **Standalone Activities** view. You'll see `deliver-evt_001` listed as a completed Standalone Activity. This is the API surface you've used through all five previous modules.
...
- **Standalone Activities** view: `deliver-evt_001` from step 2 is here.
- **Workflows** view: `wf-evt_002` is here, and clicking it shows the Activity as a step in its history.
...
> **What's happening:** the Worker registered both the Activity and the Workflow. Temporal's server routes an `ActivityClient.execute` call to dispatch the Activity directly; a Workflow submission dispatches the Workflow, which then dispatches the Activity. Same code, two submission paths, two job types.
...
- The `idConflictPolicy`, `maxWorkerActivitiesPerSecond`, and `Priority` knobs you learned still apply when those Activities are called from inside a Workflow.
...
- **`maxWorkerActivitiesPerSecond`** on the Worker: cap dispatch rate to protect the downstream service.
- **`Priority`** on the Activity: push urgent work ahead of bulk when the queue is contended.
- **`ctx.heartbeat(progress)` + `heartbeatTimeout`**: report progress from long-running jobs and resume from the last checkpoint after a crash.
