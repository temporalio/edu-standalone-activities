# Standalone Activities quickstart (.NET)
...
| File | Purpose |
|---|---|
| `Activities.cs` | A plain `Greet` Activity. |
| `Worker.cs` | Worker that connects via `ClientEnvConfig.LoadClientConnectOptions()` and polls the Task Queue. |
| `Client.cs` | Executes `Greet` as a Standalone Activity (`client.ExecuteActivityAsync`). |
| `Shared.cs` | Shared Task Queue name. |
| `Program.cs` | Entry point that dispatches `worker` vs `client`. |
| `config.toml` | Reference profiles (`default` for local, `cloud` for Temporal Cloud). |

Connection settings are never hardcoded. `LoadClientConnectOptions()` (from
`Temporalio.Common.EnvConfig`) resolves them at startup from:
...
It returns a `TemporalClientConnectOptions` for `TemporalClient.ConnectAsync`.
When no config file and no env vars are present, `TargetHost` is null, so the
Worker and client fall back to `localhost:7233`.

## Prerequisites

- .NET 8.0 or later (`dotnet --version`).
...
Dependencies (the `Temporalio` NuGet package) restore automatically on the first
`dotnet run`.
...
dotnet run worker                  # terminal 2
dotnet run client                  # terminal 3
```
...
Pass a name as an argument: `dotnet run client Angela`.
...
2. Start the Worker and run the client with the `cloud` profile selected:
   ...
   ```bash
   TEMPORAL_PROFILE=cloud dotnet run worker      # terminal 1
   TEMPORAL_PROFILE=cloud dotnet run client      # terminal 2
   ```
...
```bash
TEMPORAL_CONFIG_FILE=./config.toml TEMPORAL_PROFILE=cloud dotnet run client
```
