...
| 06 | **Same code runs anywhere** (wrap-up) | 8 min | Take the exact `deliver_webhook` Activity from Module 01 and submit it as a Standalone Activity *and* as a step inside a Workflow. Show one platform, two job types, zero rewrites. Lego-block framing. Coinbase citation. | — (capstone; conceptual rather than fail-then-fix) |
