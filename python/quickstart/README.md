# Standalone Activities quickstart (Python)
...
| File | Purpose |
|---|---|
| `activities.py` | A plain `greet` Activity. |
| `worker.py` | Worker that connects via `ClientConfig.load_client_connect_config()` and polls the Task Queue. |
| `client.py` | Executes `greet` as a Standalone Activity (`client.execute_activity`). |
| `shared.py` | Shared Task Queue name. |
| `config.toml` | Reference profiles (`default` for local, `cloud` for Temporal Cloud). |

Connection settings are never hardcoded. `load_client_connect_config()` (from
...
It returns a dict of keyword arguments for `Client.connect(**config)`. When no
config file and no env vars are present the dict is empty, so the Worker and
client fall back to `localhost:7233`.

## Prerequisites

- Python 3.10 or later.
- [uv](https://docs.astral.sh/uv/) (`brew install uv`, or see the install
  guide). `uv run` creates the virtual environment and installs dependencies
  from `pyproject.toml` / `uv.lock` on first use — no manual venv step.
...
Pass a name as an argument: `uv run client.py Angela`.
...
2. Start the Worker and run the client with the `cloud` profile selected:
   ...
   ```bash
   TEMPORAL_PROFILE=cloud uv run worker.py      # terminal 1
   TEMPORAL_PROFILE=cloud uv run client.py      # terminal 2
   ```
...
```bash
TEMPORAL_CONFIG_FILE=./config.toml TEMPORAL_PROFILE=cloud uv run client.py
```
