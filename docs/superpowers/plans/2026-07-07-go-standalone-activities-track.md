...
- Canonical tab order (mirrors the TS track exactly, so `tab-N` is uniform): `tab-0` Exercise, `tab-1` Solution, `tab-2` Terminal, `tab-3` Worker, `tab-4` Webhook receiver, `tab-5` Temporal UI, `tab-6` Interactive Diagram (M05 only, `type: service` port 9001). Exercise is listed first so it is the landing tab (matches the TS track; diverges from the AGENTS.md Python-era "Temporal UI first" note by explicit user decision on 2026-07-07).
...
- [ ] **Step 1: For each module, port its `assignment.md`** from `git show origin/add-typescript-track:typescript/instruqt/NN-<slug>/assignment.md`. Transform:
  - Frontmatter: keep `slug`, `type`, `title`, `teaser`, `notes`, `difficulty`, `timelimit`. Remove `id` and per-tab `id:` lines (first push assigns them). Retitle notes to "(Go)". Keep the SAME **tabs order as the TS assignment** (Exercise `tab-0`, Solution `tab-1`, Terminal `tab-2`, Worker `tab-3`, Webhook receiver `tab-4`, Temporal UI `tab-5`, Interactive Diagram `tab-6` in M05 only) and set each `type: code`/`terminal` tab's `path`/`workdir` to the Go seeded dirs `/root/workshop/exercise/NN-<slug>` and `/root/workshop/solution/NN-<slug>` (per Task 10 layout).
